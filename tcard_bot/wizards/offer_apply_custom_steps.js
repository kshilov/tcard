...
async function apply_dialog(ctx){
    ...
    try{
        ...
        ctx.replyWithMarkdown(ctx.i18n.t('apply_dialog_offer'), Markup.inlineKeyboard([
...
async function finished_answer(ctx) {
    ...
    if (ctx.update.callback_query && ctx.update.callback_query.data == 'answers_edit'){
        ctx.reply(ctx.i18n.t('Введите номер ответа, который необходимо исправить (например: 1)'));
        return ctx.wizard.next()
    }
...
function show_preview(ctx){
    ...
    Markup.inlineKeyboard([
        Markup.callbackButton('Исправить', 'answers_edit'),
        Markup.callbackButton('Все верно', 'answers_approve'),
    ]).extra())
...
const apply_offer_custom_dialog_wizard = new WizardScene('apply-dialog-offer-wizard',
    apply_dialog,
    apply_steps,
    progress_answer,
    finished_answer,
...
logger.info("SUCCESS wizards: apply_offer_custom_dialog_wizard initialized");


module.exports = apply_offer_custom_dialog_wizard;